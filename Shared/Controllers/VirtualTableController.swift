...
class VirtualTableController: ObservableObject {
  ...
  init() {
    manager = SocketManager(
      ...
      config: [
        .path("/rt/sio2/"), .version(.two), .forceWebsockets(true), .log(false),
        .reconnectAttempts(-1), .reconnectWaitMax(1),
      ])
      ...
      if let platforms = data[0] as? [String: Any] {
        for (_, value) in platforms {
            ...
            let tabsJson = json["tab"] as? [Any]
          {
            ...
            newTabs.removeAll(where: {
              $0.platform == _platform || $0.stopId != self.currentStop.id
            })
