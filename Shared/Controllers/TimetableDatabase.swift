...
class OfflineRoutingEngine {
    let scheduleDb: SQLiteDatabase; let baseDb: SQLiteDatabase; let timetableDb: TimetableDatabase
    init(scheduleDb: SQLiteDatabase, baseDb: SQLiteDatabase, timetableDb: TimetableDatabase) {
        self.scheduleDb = scheduleDb; self.baseDb = baseDb; self.timetableDb = timetableDb
    }
    struct StopInfo: Hashable { let id: String; let name: String; let normalizedName: String; let lat: Double; let lon: Double; let platform: String?; let feed: String }
    struct MemStopTime { let stopId: String; let arrival: Int; let departure: Int; let seq: Int }
    struct MemTrip { let id: String; let feed: String; let routeShortName: String; let routeType: Int; let headsign: String; var stopTimes: [MemStopTime] }
    struct GraphState { let stopId: String; let time: Int; let parts: [Part]; let zones: Set<String>; let transitLegs: Int; let lastTripId: String? }
    
    private func distanceBetween(lat1: Double, lon1: Double, lat2: Double, lon2: Double) -> Double {
        let r = 6371e3; let phi1 = lat1 * Double.pi / 180; let phi2 = lat2 * Double.pi / 180; let deltaPhi = (lat2 - lat1) * Double.pi / 180; let deltaLambda = (lon2 - lon1) * Double.pi / 180
        let a = sin(deltaPhi / 2) * sin(deltaPhi / 2) + cos(phi1) * cos(phi2) * sin(deltaLambda / 2) * sin(deltaLambda / 2)
        return r * 2 * atan2(sqrt(a), sqrt(1 - a))
    }
    private func normalizeStopName(_ name: String) -> String {
        return name.replacingOccurrences(of: "Bratislava, ", with: "").replacingOccurrences(of: "Rovinka, ", with: "").replacingOccurrences(of: "Dunajská Lužná, ", with: "").replacingOccurrences(of: "Miloslavov, ", with: "")
    }
    private func getStops(name: String) -> [StopInfo] {
        var searchName = name; if name == "Autobusová stanica" { searchName = "Bratislava, AS" }
        ...
        return rows.compactMap { row in
            guard let id = row["stop_id"] as? String, let sname = row["stop_name"] as? String, let lat = row["stop_lat"] as? Double, let lon = row["stop_lon"] as? Double, let feed = row["feed"] as? String else { return nil }
    ...
    func planTrip(fromName: String, toName: String, date: Date, maxTransfers: Int, maxWalkDuration: Int) -> [Journey] {
        let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]; let logFile = docDir.appendingPathComponent("routing_log.txt")
        let fromStops = getStops(name: fromName); let toStops = getStops(name: toName); let calendar = Calendar.current; let startOfSearchDay = calendar.startOfDay(for: date); let initialTimeInSeconds = max(0, Int(date.timeIntervalSince(startOfSearchDay)) - 10800)
        var logStr = "--- PLAN TRIP START ---\nFrom: \(fromName), To: \(toName)\nInitial Time: \(initialTimeInSeconds)\n"
        if fromStops.isEmpty || toStops.isEmpty { logStr += "Missing stops!\n"; try? logStr.write(to: logFile, atomically: true, encoding: .utf8); return [] }
        var journeys = [Journey](); let toStopIds = Set(toStops.map { $0.id })
        let allStopsRows = baseDb.query("SELECT stop_id, stop_name, stop_lat, stop_lon, platform_code, feed, zone_id FROM stops")
        var allStops = [String: StopInfo](); var stopsByName = [String: [StopInfo]](); var zonesByStop = [String: String]()
        for row in allStopsRows {
            ...
            allStops[id] = info; stopsByName[info.normalizedName, default: []].append(info)
            if let z = row["zone_id"] as? String, !z.isEmpty { zonesByStop[id] = z }
        }
        var currentDayOffset = 0
        while journeys.count < 5 && currentDayOffset < 2 {
            let evalDay = startOfSearchDay.addingTimeInterval(TimeInterval(currentDayOffset * 86400)); let dateStr = evalDay.toString(); let dayOfWeek = timetableDb.dayOfWeekColumn(for: evalDay)
            var sids = Set<String>(); for feed in ["city", "regional", "trains"] { sids.formUnion(timetableDb.activeServiceIds(feed: feed, date: dateStr, dayOfWeek: dayOfWeek, scheduleDb: scheduleDb)) }
            if sids.isEmpty { currentDayOffset += 1; continue }
            let placeholders = sids.map { _ in "?" }.joined(separator: ","); var params: [Any] = []; params.append(contentsOf: Array(sids))
            let q = "SELECT st.trip_id, st.stop_id, st.departure_time, st.departure_time as arrival_time, st.stop_sequence, r.route_short_name, r.route_type, t.trip_headsign, st.feed FROM stop_times st JOIN trips t ON st.trip_id = t.trip_id AND st.feed = t.feed JOIN base.routes r ON t.route_id = r.route_id AND t.feed = r.feed WHERE t.service_id IN (\(placeholders))"
            let rows = scheduleDb.query(q, params: params); var tripsDict = [String: MemTrip]()
            for row in rows {
                ...
                let adjustedDep = dep + (currentDayOffset * 86400); let adjustedArr = arr + (currentDayOffset * 86400)
                let st = MemStopTime(stopId: sId, arrival: adjustedArr, departure: adjustedDep, seq: seq)
                if tripsDict[tId] != nil { tripsDict[tId]!.stopTimes.append(st) } else { tripsDict[tId] = MemTrip(id: tId, feed: f, routeShortName: rS, routeType: rT, headsign: h, stopTimes: [st]) }
            }
            var tripsByStop = [String: [(tripId: String, seq: Int, dep: Int, stopId: String)]]()
            for (tId, trip) in tripsDict { for st in trip.stopTimes { tripsByStop[st.stopId, default: []].append((tripId: tId, seq: st.seq, dep: st.departure, stopId: st.stopId)) } }
            ...
            for fs in fromStops { startTripsAtOrigin.append(contentsOf: tripsByStop[fs.id] ?? []) }
            startTripsAtOrigin = startTripsAtOrigin.filter { $0.dep >= dayStartTime }.sorted { $0.dep < $1.dep }
            let startTripCandidates = startTripsAtOrigin.prefix(30)
            for startCandidate in startTripCandidates {
                var bestArrLocal = [String: Int](); var journeysForThisStart = [Journey]()
                ...
                for leg in 1...maxLegs {
                    ...
                    for state in activeStates {
                        ...
                        var origins = [(id: String, time: Int, part: Part?)](); origins.append((id: state.stopId, time: state.time, part: nil))
                        ...
                        for origin in origins {
                            ...
                            for pt in passingTrips {
                                ...
                                if pt.dep < origin.time || pt.dep > origin.time + 43200 { continue } // 12h wait
                                ...
                                for st in trip.stopTimes where st.seq > pt.seq {
                                    if st.arrival < (bestArrLocal[st.stopId] ?? 2000000) {
                                        ...
                                        var nParts = state.parts; if let wp = origin.part { nParts.append(wp) }
                                        ...
                                        nParts.append(p); var nZones = state.zones
                                        for tst in trip.stopTimes where tst.seq >= pt.seq && tst.seq <= st.seq { if let z = zonesByStop[tst.stopId] { nZones.insert(z) } }
                                        if toStopIds.contains(st.stopId) { journeysForThisStart.append(Journey(id: UUID().uuidString, parts: nParts, zones: Array(nZones).sorted())) }
                                        else if leg < maxLegs { nextStates.append(GraphState(stopId: st.stopId, time: st.arrival, parts: nParts, zones: nZones, transitLegs: leg, lastTripId: pt.tripId)) }
                                    }
                                }
                            }
                        }
                    }
                    activeStates = nextStates; if !journeysForThisStart.isEmpty { break }
                }
                if let best = journeysForThisStart.sorted(by: { ($0.parts?.last?.endArrival ?? Date()) < ($1.parts?.last?.endArrival ?? Date()) }).first { journeys.append(best) }
    ...
    private func unique(_ journeys: [Journey]) -> [Journey] {
        var result = [Journey](); for j in journeys { if !result.contains(where: { $0 == j }) { result.append(j) } }; return result
    }
}
